"""``_BaseDrone`` + shared module-level drone helpers.

Extracted from ``sprites.drone`` in the 2026-05-07 refactor; the
``MiningDrone`` and ``CombatDrone`` subclasses live in
``sprites.drone_mining`` and ``sprites.drone_combat`` respectively.
``sprites.drone`` itself is now a re-export shim so existing
``from sprites.drone import MiningDrone`` / ``from sprites.drone
import drone_status_label`` style imports keep working.
...
from constants import (
    DRONE_HP,
    DRONE_MAX_SPEED, DRONE_FOLLOW_DIST,
