...
- **Multi-ship system** — upgrading via "Advanced Ship" in the build menu now places a new ship in the world (placement mode with ghost ship texture) instead of swapping textures in-place. The old ship persists as a `ParkedShip` sprite in `gv._parked_ships` (arcade.SpriteList). Each parked ship stores its own `cargo_items` dict, `module_slots` list, HP, shields, faction, type, and level. Players switch ships by clicking a parked ship within 300px — `building_manager.switch_to_ship()` snapshots the current player into a new ParkedShip, creates a new PlayerShip from the target, and swaps inventory/modules/weapons/ability meter. Parked ships take damage from all sources (alien lasers, player weapons, boss), and on destruction drop cargo as pickups and modules as blueprint pickups. Parked ships are stashed with zone transitions (added to `_ZONE1_LISTS` and Zone 2's `_building_stash`), fully serialized in save/load, and shown on the minimap as teal dots.
...
- **Test infrastructure** — `pytest.ini` excludes `integration/` from default runs; `conftest.py` provides `StubPlayer` (arcade.Sprite subclass) and `StubGameView` fixtures for zone update testing without a window; `psutil` dev dependency for memory measurement in soak tests; 393 fast tests (0.6 s) + 63 integration tests across 21 fast test files and 5 integration test files (462 total). Integration breakdown: 18 functional (test_zone2_real_gv), 17 full-frame FPS (test_performance), 10 GPU render (test_render_perf), 12 resolution scaling (test_resolution_perf), 6 soak/endurance (test_soak — 5 min each, ~30 min total)
