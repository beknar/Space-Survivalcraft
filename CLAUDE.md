...
- **AI Pilot module** — craft `ai_pilot` at the Advanced Crafter (800 iron + 400 copper) and drop the resulting `mod_ai_pilot` onto a parked ship to install it. `ParkedShip.has_ai_pilot` flips on when `"ai_pilot" in module_slots`. `update_logic._update_parked_ships` gathers the active Home Station position, the current-zone alien list (Zone 1 + Zone 2 stash when active), and the boss, then calls `ParkedShip.update_ai`. `_ai_mode` toggles between `"patrol"` (counter-clockwise orbit at `AI_PILOT_PATROL_RADIUS * AI_PILOT_ORBIT_RADIUS_RATIO` ≈ 360 px via `_orbit_patrol`) and `"return"` (straight line back to the Home Station, cleared once the ship is within `AI_PILOT_HOME_ARRIVAL_DIST` of 100 px). Engaging a target cancels return mode; firing at a target flips to return when no other live target remains inside `AI_PILOT_DETECT_RANGE` (600 px). Shots go to `gv.turret_projectile_list` every `AI_PILOT_FIRE_COOLDOWN` (0.5 s) using `gv._turret_laser_tex`, so the existing turret-projectile collision handler delivers damage. A final clamp pins the ship to the patrol leash after each tick.
