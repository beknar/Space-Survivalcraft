...
| `sprites/planet.py` | ~40 | `Planet` --- indestructible landable world object (earth/frost/barren) with a collider radius; placed in the Star Maze |
| `sprites/landing_enemy.py` | ~115 | `LandingEnemy` --- spec-driven airborne enemy (Sky Worm / Cloud Drone / Thunder Worm); per-spawn shield roll, pursue + single/double fire |
| `sprites/resource_node.py` | ~52 | `ResourceNode` --- mineable surface node (rock/copper/silicon) with HP + yield, mined by mining-beam projectiles |

### Planet zones

| Module | Lines | Role |
|---|---|---|
| `zones/zone_planetary_landing.py` | ~190 | `PlanetaryLandingZone(WarpZoneBase)` --- the aerial descent; owns the 60-enemy population + projectile lists, sky backdrop, fractional-damage walls, bottom→origin / top→surface routing |
| `zones/zone_planetary_surface.py` | ~200 | `PlanetarySurfaceZone(ZoneState)` --- the on-foot surface; stashes/restores ship state on the ship↔on-foot mode swap, populates + mines resource nodes, bottom-edge lift-off |
...
- **Planet landing scene reuses the warp-zone framework** --- `PlanetaryLandingZone` subclasses `WarpZoneBase` (modeled on `EnemySpawnerWarpZone`): it inherits edge/exit detection, wall drawing, and fog, and owns its enemy + projectile SpriteLists with inline player↔enemy / player-projectile↔enemy collisions in `_update_hazards`. It overrides `_apply_zone_id_routing` (bottom → origin), `_check_walls` (fractional damage instead of full shield drain), and `_check_exits` (top → surface). The Star Maze gates the entry: `StarMazeZone._update_planet_contact` checks `"planetary_landing" in gv._module_slots` and either transitions or applies contact damage.
- **On-foot player mode-switch** --- the on-foot surface character REUSES `gv.player` rather than introducing a parallel sprite, so the camera / input / draw / weapon pipelines work unchanged. `PlanetarySurfaceZone.setup` stashes the ship's combat state (texture, scale, hp/shields/armor, guns, weapons) and flips the player into on-foot mode (`gv._on_foot = True`, surface texture, `armor = 1`, `max_shields = 0`, on-foot arsenal, `guns = 1` so the 2-weapon list cycles); `teardown` restores it byte-for-byte. Behaviour gated on `gv._on_foot` lives in `update_logic.update_movement` (branches to `PlayerShip.apply_input_on_foot` — direct WASD, 250 px/s, heading tracks movement) and `combat_helpers.apply_damage_to_player` (the flat armor reduction). Thruster / contrail / shield-regen self-disable on foot (thrust intensity stays 0, `max_shields` is 0).
