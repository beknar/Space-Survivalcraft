...
| `HUNT` | close on nearest alien (reuses `_act_engage`'s close-and-fight) | no asteroid visible AND alien within `HUNT_RANGE_PX` (3000 px) AND not in REGEN/ENGAGE/etc | asteroid appears OR alien drifts past 3000 px (then `IDLE_AT_BASE` if HS exists, else `SEARCH`) OR alien closes inside 800 px (then `ENGAGE`) |
| `IDLE_AT_BASE` | navigate to within `IDLE_AT_BASE_RADIUS_PX` (300 px) of the Home Station and idle there waiting for respawns | no asteroid visible AND no alien within 3000 px AND a Home Station exists | asteroid appears (then `MINE`) OR alien comes into 3000 px (then `HUNT`) OR alien closes to 800 px (then `ENGAGE`) |
