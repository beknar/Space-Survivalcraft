...
class SplashView(arcade.View):
        ...
        for i in range(SAVE_SLOT_COUNT):
            ...
            if os.path.exists(path):
                try:
                    ...
                    self._load_slots.append({"name": name, "exists": True})
                except Exception:
    ...
    def _load_slot_rects(self) -> list[tuple[int, int, int, int]]:
        ...
        slot_w, slot_h, gap = 340, 30, 6
        panel_h = SAVE_SLOT_COUNT * (slot_h + gap) + 100
        top_y = SCREEN_HEIGHT // 2 + panel_h // 2 - 60
        rects = []
        for i in range(SAVE_SLOT_COUNT):
            sx = (SCREEN_WIDTH - slot_w) // 2
            sy = top_y - i * (slot_h + gap)
            rects.append((sx, sy, slot_w, slot_h))
        return rects
    ...
    def _draw_load_screen(self) -> None:
        ...
        for i in range(SAVE_SLOT_COUNT):
            ...
            self._t_load_labels[i].y = sy + sh // 2
